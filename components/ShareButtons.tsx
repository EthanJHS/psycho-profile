...
export default function ShareButtons({ profileLabel, profileId: _profileId }: Props) {
  ...
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://psycho-profile.vercel.app'
  const shareText = `나의 심리 프로파일은 "${profileLabel}" 🔮\n192개 유형 중 나에게 딱 맞는 분석을 받아봤어. 너도 해봐 →`
  ...
  function shareKakao() {
    ...
    const url = `https://story.kakao.com/share?url=${encodeURIComponent(siteUrl)}`
